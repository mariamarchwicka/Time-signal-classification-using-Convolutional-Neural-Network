...
def cnn_model_fn(features, labels, mode):
  ...
  pool4 = tf.compat.v1.layers.max_pooling2d(inputs=conv4, pool_size=[2, 2], strides=2, padding="same")
  #Output -1,15,13,128

  # Dense Layer
  pool4_flat = tf.reshape(pool4, [-1, 15 * 13 * 128])
  dense = tf.compat.v1.layers.dense(inputs=pool4_flat, units=2048, activation=tf.nn.relu)

  dropout2 = tf.compat.v1.layers.dropout(
      inputs=dense, rate=0.2, training=mode == tf.estimator.ModeKeys.TRAIN)

  dense2 = tf.compat.v1.layers.dense(inputs=dropout2, units=1024, activation=tf.nn.relu)

  dropout3 = tf.compat.v1.layers.dropout(
      inputs=dense2, rate=0.2, training=mode == tf.estimator.ModeKeys.TRAIN)

  # Logits Layer
  logits = tf.compat.v1.layers.dense(inputs=dropout3, units=1)
